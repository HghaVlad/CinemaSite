...
from utlis import process_payment


class PaymentsService:

    @staticmethod
    async def book_showtime(booking: MakeBooking, user_id) -> Ticket:
        async with get_postgres_session() as session:
            showtime = await session.get(Showtime, booking.showtime_id)
            if not showtime:
                raise HTTPException(status_code=404, detail="Showtime not found")

            ticket = await session.get(Ticket, showtime.ticket_id)
            if not ticket:
                raise HTTPException(status_code=404, detail="Ticket not found")
            if ticket.is_booked:
                return HTTPException(status_code=400, detail="Ticket is already booked")
    ...
    @staticmethod
    async def pay_showtime(payment: UserPayment) -> PaymentStatus:
        async with get_postgres_session() as session:
    ...
    @staticmethod
    async def get_user_orders(user_id) -> List[OrderResponse]:
        async with get_postgres_session() as session:
...
@lru_cache
async def get_payments_service() -> PaymentsService:
